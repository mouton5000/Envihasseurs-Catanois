...
# Les joueurs recoltent leur ressources : lancement des des, autre que 7, recolte des ressources en fonction des villages et des villes, et des brigands. Recolte de l'or
    def test_recolter_ressources(self):
        ...
        with self.assertRaises(RecolteError) as err:
            self.assertFalse(JeuNight.peut_recolter_ressources(-1))
        self.assertEquals(err.exception.error_code, RecolteError.HORS_LIMITE)
        with self.assertRaises(RecolteError) as err:
            self.assertFalse(JeuNight.peut_recolter_ressources(0))
        self.assertEquals(err.exception.error_code, RecolteError.HORS_LIMITE)
        with self.assertRaises(RecolteError) as err:
            self.assertFalse(JeuNight.peut_recolter_ressources(1))
        self.assertEquals(err.exception.error_code, RecolteError.HORS_LIMITE)
        with self.assertRaises(RecolteError) as err:
            self.assertFalse(JeuNight.peut_recolter_ressources(7))
        self.assertEquals(err.exception.error_code, RecolteError.SEPT)
        with self.assertRaises(RecolteError) as err:
            self.assertFalse(JeuNight.peut_recolter_ressources(13))
        self.assertEquals(err.exception.error_code, RecolteError.HORS_LIMITE)
        self.assertTrue(JeuNight.peut_recolter_ressources(2))
        self.assertTrue(JeuNight.peut_recolter_ressources(3))
        self.assertTrue(JeuNight.peut_recolter_ressources(4))
        self.assertTrue(JeuNight.peut_recolter_ressources(5))
        self.assertTrue(JeuNight.peut_recolter_ressources(6))
        self.assertTrue(JeuNight.peut_recolter_ressources(8))
        self.assertTrue(JeuNight.peut_recolter_ressources(9))
        self.assertTrue(JeuNight.peut_recolter_ressources(10))
        self.assertTrue(JeuNight.peut_recolter_ressources(11))
        self.assertTrue(JeuNight.peut_recolter_ressources(12))

        c = Cartes.RIEN

        j1.setCartes(tg,c)
        j1.setOr(tg,0)
        JeuNight.recolter_ressources(5)
        self.assertEqual(j1.getCartes(tg),CartesRessources(1,0,0,0,1))
        self.assertEqual(j1.getOr(tg),0)
        j1.setCartes(tg,c)
        JeuNight.recolter_ressources(9)
        self.assertEqual(j1.getCartes(tg),CartesRessources(0,0,4,0,0))
        self.assertEqual(j1.getOr(tg),0)
        j1.setCartes(tg,c)
        JeuNight.recolter_ressources(6)
        ...
        v.save()
        JeuNight.recolter_ressources(3)
        self.assertEqual(j1.getCartes(tg),c)
        self.assertEqual(j1.getOr(tg),0)
        ...
        j2.setOr(tg,0)
        j2.setOr(td,0)
        JeuNight.recolter_ressources(8)
        ...
        j2.setCartes(tg,c) 
        j2.setCartes(td,c) 
        JeuNight.recolter_ressources(4)
